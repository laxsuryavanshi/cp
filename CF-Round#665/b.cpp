...
void solve_test() {
    ...
    auto ans = 0ll;
    trace(x1, y1, z1, x2, y2, z2, ans);
    auto aa = min(z2, x1);
    z2 -= aa; x1 -= aa;
    trace(aa, x1, y1, z1, x2, y2, z2, ans);
    auto bb = min(z2, z1);
    z2 -= bb; z1 -= bb;
    trace(bb, x1, y1, z1, x2, y2, z2, ans);
    auto cc = min(z2, y1);
    z2 -= cc; y1 -= cc;
    ans -= (cc * 2ll);
    trace(cc, x1, y1, z1, x2, y2, z2, ans);
    auto dd = min(z1, y2);
    z1 -= dd; y1 -= dd;
    ans += (dd * 2ll);
    trace(dd, x1, y1, z1, x2, y2, z2, ans);
    auto ee = min(y1, y2);
    y1 -= ee; y2 -= ee;
    trace(ee, x1, y1, z1, x2, y2, z2, ans);
    cout<<ans<<nl;
}
